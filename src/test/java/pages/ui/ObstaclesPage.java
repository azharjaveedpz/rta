...
public class ObstaclesPage {
	...
	public boolean navigateToAddObstaclePage() {
		return step("Navigate to Add New Inspection Page", () -> {
			...
			if (sucMsg.isEmpty())
				throw new AssertionError("Add New Inspection page message was empty");
	...
	public void validateZone(String expected) {
		step("Validate Zone", () -> validateCell(zoneCell, expected, "Zone"));
	}

	public void validateArea(String expected) {
		step("Validate Area", () -> validateCell(areaCell, expected, "Area"));
	}

	public void validateObstacle(String expected) {
		step("Validate Obstacle", () -> validateCell(obstacleCell, expected, "Obstacle"));
	}

	public void validateStatus(String expected) {
		step("Validate Status", () -> validateCell(statusCell, expected, "Status"));
	}
	...
	public boolean navigateToViewPage() {
		return step("Navigate to View page", () -> {
			...
			String sucMsg = messageElement.getText().trim();
			if (sucMsg.isEmpty())
				throw new AssertionError("View page message was empty");
	...
	public void printAndValidateObstacleCounts() {
		step("Print and validate obstacle counts", () -> {
			...
			if (total == reported + removed) {
				...
				String message = "Obstacle count validation FAILED! Total: " + total + ", Reported + Removed: "
						+ (reported + removed);
				logger.error(message);
				test.fail(message);
	...
	public void validateTableRowsAgainstTotalObstacle() {
		step("Validate table rows against total obstacle label", () -> {
			...
			if (totalFromLabel == totalFromTable) {
				...
				String message = "Validation FAILED! Total Obstacle label: " + totalFromLabel + ", Table rows: "
						+ totalFromTable;
				logger.error(message);
				test.fail(message);
		...
		step("Validate Obstacle filter list for: " + expectedObstacleSource, () -> {
			...
			for (int i = 0; i < obstacleSource.size(); i++) {
				...
				if (!actualValue.equalsIgnoreCase(expectedObstacleSource)) {
					String message = "Row " + (i + 1) + " mismatch: expected = " + expectedObstacleSource
							+ ", actual = " + actualValue;
					logger.error(message);
					test.fail(message);
					allMatched = false;
			...
			if (!allMatched) {
				throw new AssertionError(
						"One or more rows did not match the expected Obstacle Source: " + expectedObstacleSource);
			}
		...
		step("Validate Status filter list for: " + expectedStatus, () -> {
			...
			for (int i = 0; i < statusSource.size(); i++) {
				...
				if (!actualValue.equalsIgnoreCase(expectedStatus)) {
					String message = "Row " + (i + 1) + " mismatch: expected = " + expectedStatus + ", actual = "
							+ actualValue;
					logger.error(message);
					test.fail(message);
					allMatched = false;
			...
			if (!allMatched) {
				throw new AssertionError(
						"One or more rows did not match the expected Status Source: " + expectedStatus);
			}
	...
	public void validateReportedAndRemovedObstacles() {
		step("Validate Reported and Removed obstacle counts", () -> {
			...
			long reportedTable = statusList.stream().map(WebElement::getText).map(String::trim)
					.filter(status -> status.equalsIgnoreCase("Reported")).count();

			long removedTable = statusList.stream().map(WebElement::getText).map(String::trim)
